...
/******************************************************************************
 *                                                                            *
 *  ▼▼▼  PERSONAL OVERRIDES — everything below is mine and wins  ▼▼▼          *
 *                                                                            *
 *  Base above this banner: arkenfox user.js v144 (released 2026-04-20).      *
 *                                                                            *
 *  TO UPDATE ARKENFOX LATER:                                                 *
 *    1. grab the newest release ≤ your Firefox version from                  *
 *       https://github.com/arkenfox/user.js/releases                         *
 *    2. replace EVERYTHING ABOVE this banner with it                         *
 *    3. leave this block untouched at the very bottom                        *
 *                                                                            *
 ******************************************************************************/

/*** userChrome.css / userContent.css (arkenfox does not set this) ***/
user_pref("toolkit.legacyUserProfileCustomizations.stylesheets", true);

/*** Cookies: left to the Cookie AutoDelete extension (synced lists, no git/root) ***
 * Total Cookie Protection (cookieBehavior 5) still isolates third-party cookies;
 * CAD owns deletion via its whitelist/graylist. Firefox's own shutdown clearing
 * of cookies+storage is disabled so it doesn't wipe CAD's kept cookies.
 */
user_pref("privacy.clearOnShutdown_v2.cookiesAndStorage", false);

/*** Startup / search ***/
user_pref("browser.startup.homepage", "https://duckduckgo.com");
user_pref("browser.newtabpage.enabled", false);       // blank new tab instead of Firefox Home
user_pref("keyword.enabled", false);                  // no search / no guessing from the URL bar
user_pref("browser.fixup.alternate.enabled", false);  // no automatic www. + .com on hostnames
user_pref("browser.urlbar.suggest.history", false);
user_pref("browser.urlbar.suggest.openpage", false);
user_pref("browser.urlbar.suggest.topsites", false);
user_pref("browser.urlbar.trimURLs", false);          // show the full URL, not a trimmed one
// user_pref("browser.urlbar.maxRichResults", 0);     // 0 = no URL-bar dropdown at all

/*** UI / tabs ***/
user_pref("browser.uidensity", 1);                          // compact
user_pref("browser.tabs.inTitlebar", 0);                    // keep the OS title bar (ex-drawInTitlebar=false)
user_pref("browser.tabs.insertRelatedAfterCurrent", false); // new tabs go to the end
user_pref("browser.tabs.selectOwnerOnClose", false);
user_pref("browser.tabs.closeWindowWithLastTab", false);
user_pref("browser.tabs.warnOnClose", true);
user_pref("middlemouse.contentLoadURL", false);             // middle-click doesn't open the selection as a URL
user_pref("dom.popup_maximum", 3);
user_pref("layout.css.visited_links_enabled", false);       // don't style visited links differently (pointless/annoying)
user_pref("clipboard.autocopy", false);                     // don't auto-copy selected text to the X primary selection
...
/*** Logins (passwords managed externally) ***/
user_pref("signon.generation.enabled", false);
user_pref("signon.includeOtherSubdomainsInLookup", false);
user_pref("signon.firefoxRelay.feature", "disabled");       // no "Suggest Firefox Relay email masks"

/*** Extra API hardening — stricter than arkenfox defaults.
 *   Trim any line that breaks a site you use. arkenfox neutralises some of
 *   these by spoofing rather than disabling (e.g. netinfo), so a few are
 *   belt-and-suspenders. ***/
user_pref("geo.enabled", false);
...
/*** Shutdown clearing ***/
user_pref("privacy.clearOnShutdown_v2.browsingHistoryAndDownloads", true); // wipe history on close
user_pref("privacy.clearOnShutdown_v2.cache", false);                      // keep disk cache on close
